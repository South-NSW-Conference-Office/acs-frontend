...
export default function Unions() {
   ...
   const [selectedUnion, setSelectedUnion] = useState<Union | undefined>(undefined);
   const [showCreateModal, setShowCreateModal] = useState(false);
   const [showEditModal, setShowEditModal] = useState(false);
   const [showDeleteConfirm, setShowDeleteConfirm] = useState(false);
   const [unionToDelete, setUnionToDelete] = useState<Union | undefined>(undefined);
   ...
   const fetchUnions = useCallback(async () => {
      try {
         ...
         if (response && response.success) {
            ...
            const unionsArray = Array.isArray(unionsData) ? unionsData : [];
            ...
            try {
               ...
               if (conferencesResponse.success && conferencesResponse.data) {
                  ...
                  unionsArray.forEach((u) => { conferencesData[u._id] = []; });
                  ...
                  unionsArray.forEach((u) => { emptyData[u._id] = []; });
   ...
   const handleDeleteUnion = async (union: Union) => {
      try {
         const response = await UnionService.hardDeleteUnion(union._id);
         if (response.success) {
            setUnions((prev) => prev.filter((u) => u._id !== union._id));
            toast.success('Union deleted', `${union.name} has been permanently deleted.`);
         } else {
            toast.error('Failed to delete union', response.message || 'Unknown error');
         }
      } catch (error) {
         const errorMessage = error instanceof Error ? error.message : 'An unexpected error occurred';
   ...
   const filteredUnions = (unions || []).filter((union) => {
      ...
      return (
         union.name?.toLowerCase().includes(s) ||
         union.contact?.email?.toLowerCase().includes(s) ||
         union.headquarters?.city?.toLowerCase().includes(s) ||
         union.headquarters?.country?.toLowerCase().includes(s)
      );
   });
   ...
   const toggleSelectAll = () => {
      if (selectedIds.size === filteredUnions.length) {
         setSelectedIds(new Set());
      } else {
         setSelectedIds(new Set(filteredUnions.map((u) => u._id)));
      }
   };
   ...
   const handleBulkDelete = async () => {
      ...
      for (const id of ids) {
         try {
            const response = await UnionService.hardDeleteUnion(id);
            if (response.success) {
               results.success++;
            } else {
               results.failed++;
               results.errors.push(response.message || `Failed to delete union ${id}`);
            }
         } catch (error) {
            results.failed++;
            results.errors.push(error instanceof Error ? error.message : `Failed to delete union ${id}`);
         }
      }

      if (results.success > 0) {
         setUnions((prev) => prev.filter((u) => !selectedIds.has(u._id) || results.errors.some((e) => e.includes(u._id))));
         toast.success('Bulk delete complete', `${results.success} union(s) permanently deleted.`);
      }
      if (results.failed > 0) {
         toast.error('Some deletions failed', `${results.failed} union(s) could not be deleted. ${results.errors[0]}`);
      }
   ...
   const handleUnionSaved = (savedUnion: Union, isEdit: boolean) => {
      if (isEdit) {
         setUnions((prev) => prev.map((u) => u._id === savedUnion._id ? savedUnion : u));
         toast.success('Union updated', `${savedUnion.name} has been successfully updated.`);
      } else {
         setUnions((prev) => [...prev, savedUnion]);
         toast.success('Union created', `${savedUnion.name} has been successfully created.`);
      }
   ...
   const totalConfs = Object.values(unionConferences).reduce((acc, arr) => acc + arr.length, 0);
      ...
      >
            ...
            <header className="reg-masthead">
               <div className="reg-mast-left">
                  ...
                  <h1 className="reg-title">
                     ...
                     <span className="reg-word reg-word--italic" style={{ animationDelay: '0.42s' }}>Unions</span>
                  </h1>
                  <p className="reg-subtitle">
                     An index of the administrative bodies by which conferences,
                     churches, teams, and community services are held in trust.
                  </p>
               </div>

               <aside className="reg-mast-right">
                  <div className="reg-counter">
                     <div className="reg-counter-num">{String(filteredUnions.length).padStart(2, '0')}</div>
                     <div className="reg-counter-label">Entries on Record</div>
                     <div className="reg-counter-rule" />
                     <div className="reg-counter-sub">
                        <span>{totalConfs}</span> conferences held
                     </div>
                  </div>
               </aside>
            </header>
            ...
            <div className="reg-console">
               <div className="reg-console-left">
                  <div className="reg-search">
                     ...
                     <input
                        type="text"
                        placeholder="Search the register by name, city, or correspondent…"
                        value={searchQuery}
            ...
            {loading ? (
               <div className="reg-skel-table">
                  <div className="reg-skel-header">
                     ...
                     <div className="reg-skel-line reg-skel-line--head" style={{ width: '20%' }} />
               ...
               <div className="reg-empty">
                  ...
                  <p className="reg-empty-body">
                     {searchQuery
                        ...
                        : 'Begin by inscribing the first union to the register.'}
                  </p>
               </div>
            ) : (
                        ...
                        <tr>
                           <th className="reg-th reg-th--check" onClick={(e) => e.stopPropagation()}>
                              <label className="reg-check" onClick={(e) => { e.preventDefault(); toggleSelectAll(); }}>
                                 <input
                                    type="checkbox"
                                    checked={filteredUnions.length > 0 && selectedIds.size === filteredUnions.length}
                                    readOnly
                                 />
                           ...
                           <th className="reg-th reg-th--num">Conferences</th>
                           <th className="reg-th">Standing</th>
                           <th className="reg-th reg-th--actions" aria-label="Actions" />
                        </tr>
                     </thead>
                     <tbody>
                        {filteredUnions.map((union, i) => {
                           const count = (unionConferences[union._id] || []).length;
                           const goto = () => {
                              if (showDeleteConfirm || showEditModal || showCreateModal || showBulkDeleteConfirm) return;
                              if (selectedIds.size > 0) { toggleSelect(union._id); return; }
                              window.location.href = `/unions/${union._id}`;
                           };
                           return (
                              <tr
                                 key={union._id}
                              ...
                              >
                                 <td className="reg-td reg-td--check" onClick={(e) => e.stopPropagation()}>
                                    <label className="reg-check" onClick={(e) => { e.preventDefault(); toggleSelect(union._id); }}>
                                       <input
                                          type="checkbox"
                                          checked={selectedIds.has(union._id)}
                                          readOnly
                                       />
                                       <span className="reg-check-box">
                                          <svg viewBox="0 0 12 12" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
                                          ...
                                          <div className="reg-avatar-inner">
                                             {union.primaryImage?.url ? (
                                                ...
                                                <div className="reg-avatar-fallback">
                                                   <span>{(union.name || 'U').charAt(0)}</span>
                                                </div>
                                             )}
                                          </div>
                                       </div>
                                       <div className="reg-td-union-text">
                                          <div className="reg-td-name">{union.name}</div>
                                          <div className="reg-td-path">{union.hierarchyPath}</div>
                                       </div>
                                    </div>
                                 </td>
                                 ...
                                 <td className="reg-td reg-td--seat">
                                    {(union.headquarters?.city || union.headquarters?.country) ? (
                                       <>
                                          ...
                                          <div className="reg-td-country">{union.headquarters?.country}</div>
                                       </>
                                    ) : (
                                 ...
                                 <td className="reg-td reg-td--corr">
                                    {union.contact?.email ? (
                                       <div className="reg-td-email">{union.contact.email}</div>
                                    ) : (
                                       <span className="reg-dash">&mdash;</span>
                                    )}
                                    {union.contact?.phone && (
                                       <div className="reg-td-phone">{union.contact.phone}</div>
                                    )}
                                 </td>
                                 ...
                                 <td className="reg-td reg-td--state">
                                    <span className={`reg-state ${union.isActive ? 'on' : 'off'}`}>
         ...
         <UnionModal
            ...
            <DeleteUnionDialog
               ...
               union={unionToDelete}
               conferenceCount={unionToDelete ? (unionConferences[unionToDelete._id]?.length || 0) : 0}
               onCancel={() => {
                  setShowDeleteConfirm(false);
                  setUnionToDelete(undefined);
               }}
               onConfirm={() => unionToDelete && handleDeleteUnion(unionToDelete)}
            />
      </AdminLayout>
   );
}
         ...
         <div className="del-dialog" onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true">
            ...
            <div className="del-head">
               ...
               <h2 className="del-title">
                  Permanently delete <em>{count}</em> {count === 1 ? 'union' : 'unions'}?
               </h2>
               <p className="del-lede">
                  This action is permanent and irreversible. All selected entries shall be
                  removed entirely from the database. Unions with subordinate entities
                  will be skipped.
               </p>
            </div>
            ...
            <div className="del-foot">
               ...
               >
                  {loading ? (
                     ...
                     <>
                        <span>Delete {count} {count === 1 ? 'Union' : 'Unions'}</span>
...
function DeleteUnionDialog({
   ...
   const canDelete = conferenceCount === 0;
   ...
   const requirements = [
      { label: 'Conferences',            count: conferenceCount, ready: conferenceCount === 0 },
      { label: 'Churches in this union', count: null,            ready: null },
      { label: 'Teams under those',      count: null,            ready: null },
      { label: 'Services beneath',       count: null,            ready: null },
   ];
      ...
      <div className="del-overlay" onClick={() => !loading && onCancel()}>
         <div className="del-dialog" onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true">
            ...
            <div className="del-head">
               ...
               <h2 className="del-title">
                  Strike <em>{union.name}</em> from the register?
               </h2>
               <p className="del-lede">
            ...
            <div className="del-reqs">
               ...
               {!canDelete && (
                  <p className="del-reqs-warn">
                     ...
                     There {conferenceCount === 1 ? 'is' : 'are'} <strong>{conferenceCount}</strong>
                     &nbsp;{conferenceCount === 1 ? 'conference' : 'conferences'} held by this union.
   ...
   return (
      <style jsx global>{`
         ...
         .reg-card {
            position: relative;
            opacity: 0;
            animation: reg-card-in 1.1s cubic-bezier(0.2, 0.8, 0.2, 1) both;
         }
         ...
         .reg-card-link {
            ...
            position: relative;
            overflow: hidden;
            transition: border-color 0.5s, transform 0.6s, box-shadow 0.6s;
         }
         .reg-card-link::before {
            content: ''; position: absolute; top: 0; left: 0; right: 0;
            height: 2px;
            background: linear-gradient(90deg, var(--reg-gold), transparent);
            transform: scaleX(0);
         ...
         .reg-card-link:hover {
            ...
            box-shadow:
               0 1px 0 rgba(255, 255, 255, 0.8) inset,
               0 20px 50px -24px rgba(107, 79, 21, 0.28);
         }
         .reg-card-link:hover::before { transform: scaleX(1); }
         ...
         .reg-avatar-ring--2 {
            inset: -11px;
            border-style: dotted;
            border-color: var(--reg-gold-d);
            opacity: 0.3;
         }
         .reg-card-link:hover .reg-avatar-ring         { opacity: 1; transform: rotate(18deg); }
         .reg-card-link:hover .reg-avatar-ring--2      { opacity: 0.55; transform: rotate(-18deg); }
         .reg-avatar-inner {
            position: relative;
            width: 100%; height: 100%;
            border-radius: 50%;
         ...
         .reg-card-link:hover .reg-avatar-img {
         ...
         .reg-avatar-state.on {
            ...
            box-shadow: 0 0 0 2px rgba(107, 125, 58, 0.18);
         }
         ...
         .reg-card-link:hover .reg-card-title {
            color: var(--reg-gold-d);
         }
         ...
         .reg-card-loc--empty { color: var(--reg-ink-3); }
         ...
         .reg-card-num {
            ...
            font-weight: 200;
            line-height: 1;
            color: var(--reg-gold-l) !important;
            font-feature-settings: "lnum" 1, "tnum" 1;
            letter-spacing: -0.02em;
         }
         .reg-card-dl-row--email dd {
            font-size: 0.82rem !important;
            color: var(--reg-ink-2) !important;
            max-width: 18ch;
         ...
         .reg-card-foot {
            margin-top: 1.3rem;
            padding-top: 1rem;
            border-top: 1px solid rgba(168, 127, 43, 0.15);
         }
         .reg-card-view {
            font-size: 0.68rem;
         ...
         .reg-card-link:hover .reg-card-arrow {
         ...
         .reg-avatar--sm .reg-avatar-fallback {
            font-size: 1rem;
         }
