...
export default function Services() {
   ...
   const handleDeleteService = async (service: Service) => {
      try {
         ...
         success(`${service.name} has been retired from the register.`);
      } catch {
         error('Failed to retire service');
      } finally {
   ...
   const handleServiceSaved = (savedService: Service, isEdit: boolean) => {
      ...
      if (isEdit) {
         ...
         success(`${savedService.name} has been amended.`);
      } else {
         setServices((prev) => [...prev, savedService]);
         success(`${savedService.name} has been inscribed to the register.`);
      }
   ...
   const toggleSelectAll = () => {
      if (selectedIds.size === filteredServices.length) {
         ...
         setSelectedIds(new Set(filteredServices.map((s) => s._id)));
      }
   };
   ...
   const handleBulkDelete = async () => {
      ...
      for (const id of ids) {
         try {
            await serviceManagement.deleteService(id);
            results.success++;
         } catch (err) {
            results.failed++;
      ...
      if (results.success > 0) success(`${results.success} service(s) retired from the register.`);
      if (results.failed > 0) error(`${results.failed} could not be retired. ${results.errors[0] || ''}`);
      setSelectedIds(new Set());
   ...
   const filteredServices = services.filter((service) => {
      if (!service || !service.name || !service.type) return false;
      if (!showInactive && service.status !== 'active') return false;
      if (!searchQuery.trim()) return true;
      const q = searchQuery.toLowerCase();
      const teamName = typeof service.teamId === 'object' ? service.teamId?.name : undefined;
      return (
         service.name.toLowerCase().includes(q) ||
         service.type.toLowerCase().includes(q) ||
         (service.descriptionShort?.toLowerCase().includes(q) ?? false) ||
         (teamName?.toLowerCase().includes(q) ?? false) ||
         (service.locations?.[0]?.address?.suburb?.toLowerCase().includes(q) ?? false)
      );
   });

   const distinctTeams = new Set(
      filteredServices.map((s) => typeof s.teamId === 'object' ? s.teamId?._id : s.teamId).filter(Boolean)
   ).size;
   ...
   return (
      ...
      >
            ...
            <header className="reg-masthead">
               <div className="reg-mast-left">
                  <p className="reg-kicker">
                     ...
                     Seventh&#8209;day Adventist Church&nbsp;&middot;&nbsp;Ministry
                  </p>
                  <h1 className="reg-title">
                     ...
                     <span className="reg-word reg-word--italic" style={{ animationDelay: '0.42s' }}>Ministries</span>
                  </h1>
                  <p className="reg-subtitle">
                     A record of the services rendered to the community &mdash; the
                     works of mercy, welcome, and provision kept by the teams of the church.
                  </p>
               </div>

               <aside className="reg-mast-right">
                  <div className="reg-counter">
                     <div className="reg-counter-num">{String(filteredServices.length).padStart(2, '0')}</div>
                     <div className="reg-counter-label">Ministries on Record</div>
                     <div className="reg-counter-rule" />
                     <div className="reg-counter-sub">
                        <span>{distinctTeams}</span> teams kept
                     </div>
                  </div>
               </aside>
            </header>
            ...
            <div className="reg-console">
               <div className="reg-console-left">
                  <div className="reg-search">
                     ...
                     <input
                        type="text"
                        placeholder="Search by name, kind, team, or locality…"
                        value={searchQuery}
                  ...
                  <label className="reg-toggle">
                     ...
                     <span className="reg-toggle-track"><span className="reg-toggle-thumb" /></span>
                     <span className="reg-toggle-label">Show paused &amp; archived</span>
                  </label>
               </div>
               ...
               <PermissionGate permission="services.create">
                  <button onClick={() => setShowCreateModal(true)} className="reg-btn">
                     ...
                     <span>Inscribe New Ministry</span>
                  </button>
               </PermissionGate>
            </div>
            ...
            {loading ? (
               ...
               <div className="reg-empty">
                  ...
                  <h3 className="reg-empty-title">
                     {searchQuery ? 'No ministry meets your search.' : 'The register awaits its first inscription.'}
                  </h3>
                  <p className="reg-empty-body">
                     {searchQuery
                        ? 'Amend your query, or broaden it to include paused and archived works.'
                        : 'Begin by inscribing the first ministry to the register.'}
                  </p>
               </div>
            ) : (
                  ...
                  <table className="reg-table">
                        ...
                        <tr>
                           <th className="reg-th reg-th--check" onClick={(e) => e.stopPropagation()}>
                              <label className="reg-check" onClick={(e) => { e.preventDefault(); toggleSelectAll(); }}>
                                 <input type="checkbox" checked={filteredServices.length > 0 && selectedIds.size === filteredServices.length} readOnly />
                     ...
                     <tbody>
                        {filteredServices.map((service, i) => {
                           const teamName = typeof service.teamId === 'object' ? service.teamId?.name : undefined;
                           const teamId = typeof service.teamId === 'object' ? service.teamId?._id : (typeof service.teamId === 'string' ? service.teamId : undefined);
                           const loc = service.locations?.[0];
                           const locality = loc
                              ? (loc.address.suburb || loc.label)
                              : null;
                           const localityRegion = loc?.address.state;
                           ...
                           const goto = () => {
                              ...
                              if (selectedIds.size > 0) { toggleSelect(service._id); return; }
                              router.push(`/services/${service._id}`);
                           };
                           return (
                              <tr key={service._id} className="reg-tr" style={{ animationDelay: `${0.05 + i * 0.04}s` }} onClick={goto}>
                                 <td className="reg-td reg-td--check" onClick={(e) => e.stopPropagation()}>
                                    <label className="reg-check" onClick={(e) => { e.preventDefault(); toggleSelect(service._id); }}>
                                       <input type="checkbox" checked={selectedIds.has(service._id)} readOnly />
                                       <span className="reg-check-box">
                                          <svg viewBox="0 0 12 12" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
                                             <polyline points="2.5 6 5 8.5 9.5 3.5" />
                                          </svg>
                                          ...
                                          <div className="reg-avatar-inner">
                                             {service.primaryImage?.url ? (
                                                ...
                                                <div className="reg-avatar-fallback">
                                                   <span>{(service.name || 'S').charAt(0)}</span>
                                                </div>
                                             )}
                                          </div>
                                       </div>
                                       <div className="reg-td-union-text">
                                          <div className="reg-td-name">{service.name}</div>
                                          {service.descriptionShort && (
                                             <div className="reg-td-path">{service.descriptionShort}</div>
                                          )}
                                       </div>
                                    </div>
                                 </td>
                                 ...
                                 <td className="reg-td reg-td--kind">
                                    <span className="reg-kind-chip">{service.type.replace(/_/g, ' ')}</span>
                                 </td>

                                 <td className="reg-td reg-td--parent">
                                    {teamName ? (
                                       <button
                                          onClick={(e) => { e.stopPropagation(); if (teamId) router.push(`/teams/${teamId}`); }}
                                          className="reg-td-parent reg-td-parent--link"
                                          disabled={!teamId}
                                       >
                                          {teamName}
                                       </button>
                                 ...
                                 <td className="reg-td reg-td--seat">
                                    {locality ? (
                                       <>
                                          <div className="reg-td-city">{locality}{localityRegion ? `, ${localityRegion}` : ''}</div>
                                          {loc?.address.postcode && <div className="reg-td-country">{loc.address.postcode}</div>}
                                       </>
                                    ) : (
                                       <span className="reg-dash">&mdash;</span>
                                    )}
                                 ...
                                 <td className="reg-td reg-td--state">
                                    <span className={`reg-state ${statusKey}`}>
                                 ...
                                 <td className="reg-td reg-td--actions" onClick={(e) => e.stopPropagation()}>
                                    <RowActionsMenu actions={[
                                       { label: 'View Details', onClick: () => router.push(`/services/${service._id}`) },
                                       { label: 'Edit', onClick: () => { setSelectedService(service); setShowEditModal(true); } },
                                       { label: 'Retire', onClick: () => { setServiceToDelete(service); setShowDeleteConfirm(true); }, variant: 'danger' },
                                    ]} />
                                 </td>
                              </tr>
                           );
         ...
         <ServiceModal
            ...
            onClose={() => { setShowCreateModal(false); setShowEditModal(false); setSelectedService(null); }}
         ...
         <DeleteServiceDialog
            ...
            onConfirm={() => { if (serviceToDelete) return handleDeleteService(serviceToDelete); }}
         />
      </AdminLayout>
   );
}
      ...
      <div className="del-overlay" onClick={() => !loading && onCancel()}>
         <div className="del-dialog" onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true">
            ...
            <div className="del-head">
               ...
               <h2 className="del-title">Retire <em>{count}</em> {count === 1 ? 'ministry' : 'ministries'}?</h2>
               <p className="del-lede">
                  The selected works shall be struck from the active register.
                  Their records are retained in the archive.
               </p>
            </div>
            <div className="del-foot">
               ...
               <button type="button" onClick={onConfirm} disabled={loading} className="del-btn del-btn--danger">
                  {loading ? (<><span className="del-spinner" /><span>Striking from record…</span></>) : (<><span>Retire {count} {count === 1 ? 'Ministry' : 'Ministries'}</span><span className="del-btn-arrow">&rarr;</span></>)}
               </button>
            </div>
         </div>
         <DeleteDialogStyles />
...
function DeleteServiceDialog({
   isOpen, service, onCancel, onConfirm,
}: {
   isOpen: boolean;
   ...
   const notes = [
      { label: 'Locations on record', hint: locCount > 0 ? `${locCount} noted` : 'none noted' },
      { label: 'Gallery images',      hint: galleryCount > 0 ? `${galleryCount} retained` : 'none noted' },
      { label: 'Scheduled events',    hint: 'retained in archive' },
   ];
      ...
      <div className="del-overlay" onClick={() => !loading && onCancel()}>
         <div className="del-dialog" onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true">
            ...
            <div className="del-head">
               ...
               <h2 className="del-title">Retire <em>{service.name}</em> from the register?</h2>
               <p className="del-lede">
                  The ministry shall be struck from the active register. Its record
                  is retained in the archive and may be reinstated by an administrator.
               </p>
            </div>
            ...
            <div className="del-foot">
               ...
               <button type="button" onClick={handleConfirm} disabled={loading} className="del-btn del-btn--danger">
                  {loading ? (<><span className="del-spinner" /><span>Striking from record…</span></>) : (<><span>Retire Ministry</span><span className="del-btn-arrow">&rarr;</span></>)}
               </button>
            </div>
         </div>
         <DeleteDialogStyles />
...
function DeleteDialogStyles() {
   return (<style jsx global>{`
      ...
      .del-close {
         ...
         border: 1px solid rgba(20,18,16,0.15); cursor: pointer;
         display: inline-flex; align-items: center; justify-content: center;
         transition: border-color 0.3s, background 0.3s, transform 0.3s; z-index: 2;
      }
      ...
      .del-seal-ring {
         ...
         opacity: 0.6; animation: del-rotate 18s linear infinite;
      }
...
function RegisterStyles() {
   return (<style jsx global>{`
      ...
      .reg-masthead {
         ...
         padding-bottom: 2.5rem; border-bottom: 1px solid rgba(20,18,16,0.12);
      ...
      .reg-toggle-track { position: relative; width: 34px; height: 18px; border: 1px solid var(--reg-ink-3); border-radius: 999px; background: transparent; transition: border-color 0.3s, background 0.3s; }
      .reg-toggle-thumb { position: absolute; top: 1px; left: 1px; width: 14px; height: 14px; border-radius: 50%; background: var(--reg-ink-3); transition: transform 0.35s cubic-bezier(0.2,0.8,0.2,1), background 0.3s; }
      .reg-toggle input:checked + .reg-toggle-track { border-color: var(--reg-gold); background: rgba(168,127,43,0.08); }
      .reg-toggle input:checked + .reg-toggle-track .reg-toggle-thumb { transform: translateX(16px); background: var(--reg-gold); }
      .reg-toggle-label { font-size: 0.72rem; letter-spacing: 0.2em; text-transform: uppercase; color: var(--reg-ink-2); font-weight: 500; }
      ...
      .reg-table { width: 100%; border-collapse: separate; border-spacing: 0; font-family: var(--reg-font); min-width: 960px; }
      ...
      .reg-avatar-img { width: 100%; height: 100%; object-fit: cover; filter: sepia(0.2) saturate(0.85); transition: filter 0.8s, transform 1.2s; }
      ...
      .reg-tr:hover .reg-avatar--sm .reg-avatar-img { filter: sepia(0.05) saturate(1); transform: scale(1.06); }
      ...
      .reg-td-path { font-size: 0.72rem; color: var(--reg-ink-3); font-weight: 300; letter-spacing: 0.02em; font-style: italic; max-width: 30ch; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }

      .reg-td--kind { width: 170px; }
      .reg-kind-chip { display: inline-block; font-size: 0.6rem; letter-spacing: 0.22em; text-transform: uppercase; font-weight: 500; color: var(--reg-gold-d); border: 1px solid rgba(168,127,43,0.4); background: rgba(168,127,43,0.06); padding: 0.3rem 0.7rem; }

      .reg-td--parent { min-width: 180px; }
      .reg-td-parent { font-size: 0.9rem; font-weight: 400; color: var(--reg-ink-2); font-style: italic; transition: color 0.3s; }
      .reg-tr:hover .reg-td-parent { color: var(--reg-gold-d); }
      .reg-td-parent--link { background: none; border: none; padding: 0; font: inherit; cursor: pointer; text-align: left; }
      .reg-td-parent--link:hover { color: var(--reg-gold); text-decoration: underline; text-underline-offset: 3px; text-decoration-color: var(--reg-gold-d); text-decoration-thickness: 0.5px; }
      .reg-td-parent--link:disabled { cursor: default; }
      ...
      .reg-state.on .reg-state-dot { background: var(--reg-green); box-shadow: 0 0 8px rgba(107,125,58,0.45); animation: reg-ember 2.6s ease-in-out infinite; }
      .reg-state.paused { color: var(--reg-amber); }
      .reg-state.paused .reg-state-dot { background: var(--reg-amber); box-shadow: 0 0 8px rgba(193,138,43,0.45); }
      @keyframes reg-ember { 0%, 100% { box-shadow: 0 0 8px rgba(107,125,58,0.45); } 50% { box-shadow: 0 0 4px rgba(107,125,58,0.18); } }
