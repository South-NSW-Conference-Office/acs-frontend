...
export default function ServiceTypesPage() {
   ...
   const loadServiceTypes = useCallback(async () => {
      try {
         setLoading(true);
         const types = await serviceTypeAPI.getAll({ isActive: filterActive });
   ...
   const handleCreate = () => { setEditingType(null); setModalOpen(true); };
   const handleEdit = (t: ServiceType) => { setEditingType(t); setModalOpen(true); };
   const handleDelete = (t: ServiceType) => { setServiceTypeToDelete(t); setShowDeleteConfirm(true); };

   const handleConfirmDelete = async () => {
      if (!serviceTypeToDelete) return;
      try {
         await serviceTypeAPI.delete(serviceTypeToDelete._id);
         showSuccess('Service kind retired from the register');
         loadServiceTypes();
      } catch (error) {
         const errorMessage = (error as { response?: { data?: { message?: string } } }).response?.data?.message;
         showError('Failed to retire service kind', errorMessage);
      } finally {
   ...
   const handleSubmit = async (data: ServiceTypeFormData) => {
      try {
         ...
         if (editingType) {
            ...
            showSuccess('Service kind amended');
         } else {
            await serviceTypeAPI.create(data);
            showSuccess('Service kind inscribed');
         }
         setModalOpen(false);
         loadServiceTypes();
      } catch (error) {
         const errorMessage = (error as { response?: { data?: { message?: string } } }).response?.data?.message;
         showError('Failed to save service kind', errorMessage);
      } finally {
         setSubmitting(false);
      }
   };
   ...
   const toggleSelectAll = () => {
      if (selectedIds.size === filteredTypes.length) {
         ...
         setSelectedIds(new Set(filteredTypes.map((t) => t._id)));
      }
   };
   ...
   const handleBulkDelete = async () => {
      ...
      for (const id of ids) {
         try {
            ...
            results.failed++;
            const errorMessage = (error as { response?: { data?: { message?: string } } }).response?.data?.message;
            results.errors.push(errorMessage || `Failed ${id}`);
         }
      }
      if (results.success > 0) showSuccess(`${results.success} service kind(s) retired from the register.`);
      if (results.failed > 0) showError(`${results.failed} could not be retired. ${results.errors[0] || ''}`);
      setSelectedIds(new Set());
   ...
   if (!hasPermission) {
      return (
         <AdminLayout title="Service Types" hideTitle={true} hideHeader={true}>
            <RegisterStyles />
            <div className="reg-root">
               ...
               <div className="reg-empty" style={{ marginTop: '6rem' }}>
                  <div className="reg-empty-seal">
                     <span className="reg-empty-glyph">&#10022;</span>
                  </div>
                  <h3 className="reg-empty-title">The register is closed to you.</h3>
                  <p className="reg-empty-body">You do not hold the warrant to manage service kinds. Petition your administrator for access.</p>
               </div>
            </div>
         </AdminLayout>
      );
   ...
   const activeTotal = serviceTypes.filter((t) => t.isActive).length;
   ...
   return (
      <AdminLayout title="Service Types" hideTitle={true} hideHeader={true}>
            ...
            <header className="reg-masthead">
               <div className="reg-mast-left">
                  <p className="reg-kicker">
                     ...
                     Seventh&#8209;day Adventist Church&nbsp;&middot;&nbsp;Classification
                  </p>
                  <h1 className="reg-title">
                     ...
                     <span className="reg-word reg-word--italic" style={{ animationDelay: '0.42s' }}>Service&nbsp;Kinds</span>
                  </h1>
                  <p className="reg-subtitle">
                     A taxonomy of the kinds of service rendered to the community &mdash;
                     by which every ministry is classed in the register.
                  </p>
               </div>

               <aside className="reg-mast-right">
                  <div className="reg-counter">
                     <div className="reg-counter-num">{String(filteredTypes.length).padStart(2, '0')}</div>
                     <div className="reg-counter-label">Kinds on Record</div>
                     <div className="reg-counter-rule" />
                     <div className="reg-counter-sub">
                        <span>{activeTotal}</span> in standing
                     </div>
                  </div>
               </aside>
            </header>
            ...
            <div className="reg-console">
               <div className="reg-console-left">
                  <div className="reg-search">
                     ...
                     <input
                        type="text"
                        placeholder="Search kinds by name, value, or designation…"
                        value={searchTerm}
                  ...
                  <div className="reg-filter">
                     <label className="reg-filter-label" htmlFor="filter-active">Status</label>
                     <select
                        id="filter-active"
                     ...
                     >
                        ...
                        <option value="true">In Standing</option>
                        <option value="false">Dormant</option>
                     </select>
                  </div>
               </div>

               <button onClick={handleCreate} className="reg-btn">
                  <span className="reg-btn-plus">+</span>
                  <span>Inscribe New Kind</span>
               </button>
            </div>
            ...
            {loading ? (
               <div className="reg-skel-table">
                  <div className="reg-skel-header">
                     ...
                     <div className="reg-skel-line reg-skel-line--head" style={{ width: '28%' }} />
                  ...
                  {[0, 1, 2, 3, 4].map((i) => (
                     <div key={i} className="reg-skel-row" style={{ animationDelay: `${i * 0.08}s` }}>
                        ...
                        <div className="reg-skel-line" style={{ flex: 2.2 }} />
               ...
               <div className="reg-empty">
                  ...
                  <h3 className="reg-empty-title">
                     {searchTerm ? 'No kind meets your search.' : 'The register awaits its first kind.'}
                  </h3>
                  <p className="reg-empty-body">
                     {searchTerm ? 'Amend your query, or clear the status filter.' : 'Begin by inscribing the first service kind to the register.'}
                  </p>
               </div>
            ) : (
                  ...
                  <table className="reg-table">
                        ...
                        <tr>
                           <th className="reg-th reg-th--check" onClick={(e) => e.stopPropagation()}>
                              <label className="reg-check" onClick={(e) => { e.preventDefault(); toggleSelectAll(); }}>
                                 <input type="checkbox" checked={filteredTypes.length > 0 && selectedIds.size === filteredTypes.length} readOnly />
                           ...
                           <th className="reg-th">Value</th>
                           <th className="reg-th">Designation</th>
                     ...
                     <tbody>
                        {filteredTypes.map((t, i) => {
                           const goto = () => {
                              if (showDeleteConfirm || modalOpen || showBulkDeleteConfirm) return;
                              if (selectedIds.size > 0) { toggleSelect(t._id); return; }
                              handleEdit(t);
                           };
                           return (
                              <tr key={t._id} className="reg-tr" style={{ animationDelay: `${0.05 + i * 0.04}s` }} onClick={goto}>
                                 <td className="reg-td reg-td--check" onClick={(e) => e.stopPropagation()}>
                                    <label className="reg-check" onClick={(e) => { e.preventDefault(); toggleSelect(t._id); }}>
                                       <input type="checkbox" checked={selectedIds.has(t._id)} readOnly />
                                       <span className="reg-check-box">
                                          <svg viewBox="0 0 12 12" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
                                             <polyline points="2.5 6 5 8.5 9.5 3.5" />
                                          </svg>
                                 ...
                                 <td className="reg-td reg-td--no"><span className="reg-no">{String(t.displayOrder || i + 1).padStart(2, '0')}</span></td>
                                       ...
                                       <div className="reg-avatar reg-avatar--sm">
                                          ...
                                          <div className="reg-avatar-inner">
                                             <div className="reg-avatar-fallback">
                                                <span>{(t.name || 'K').charAt(0).toUpperCase()}</span>
                                             </div>
                                          </div>
                                       </div>
                                       <div className="reg-td-union-text">
                                          <div className="reg-td-name">{t.name}</div>
                                       </div>
                                    </div>
                                 </td>
                                 <td className="reg-td reg-td--value">
                                    <code className="reg-code">{t.value}</code>
                                 </td>
                                 <td className="reg-td reg-td--desc">
                                    {t.description ? (
                                       <div className="reg-td-desc">{t.description}</div>
                                    ) : (
                                       <span className="reg-dash">&mdash;</span>
                                    )}
                                 </td>
                                 <td className="reg-td reg-td--state">
                                    <span className={`reg-state ${t.isActive ? 'on' : 'off'}`}>
                                       <span className="reg-state-dot" />
                                       {t.isActive ? 'In Standing' : 'Dormant'}
                                    </span>
                                 </td>
                                 <td className="reg-td reg-td--actions" onClick={(e) => e.stopPropagation()}>
                                    <RowActionsMenu actions={[
                                       { label: 'Edit', onClick: () => handleEdit(t) },
                                       { label: 'Retire', onClick: () => handleDelete(t), variant: 'danger' },
                                    ]} />
                                 </td>
                              </tr>
                           );
         ...
         <DeleteServiceTypeDialog
            ...
            onCancel={() => { setShowDeleteConfirm(false); setServiceTypeToDelete(null); }}
      ...
      <div className="del-overlay" onClick={() => !loading && onCancel()}>
         <div className="del-dialog" onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true">
            ...
            <div className="del-head">
               ...
               <h2 className="del-title">Retire <em>{count}</em> {count === 1 ? 'kind' : 'kinds'}?</h2>
               <p className="del-lede">Services classed by these kinds may require re&#8209;classification before they are retired.</p>
            </div>
            <div className="del-foot">
               ...
               <button type="button" onClick={onConfirm} disabled={loading} className="del-btn del-btn--danger">
                  {loading ? (<><span className="del-spinner" /><span>Striking from record…</span></>) : (<><span>Retire {count} {count === 1 ? 'Kind' : 'Kinds'}</span><span className="del-btn-arrow">&rarr;</span></>)}
               </button>
            </div>
         </div>
         <DeleteDialogStyles />
...
function DeleteServiceTypeDialog({
   isOpen, serviceType, onCancel, onConfirm,
}: {
   isOpen: boolean;
   ...
   if (!isOpen || !serviceType || !mounted) return null;
   const handleConfirm = async () => { setLoading(true); try { await onConfirm(); } finally { setLoading(false); } };
   const notes = [
      { label: 'Value code',                   hint: serviceType.value },
      { label: 'Services bearing this kind',   hint: 'to be re-classified' },
      { label: 'Display order',                hint: String(serviceType.displayOrder) },
   ];
   return createPortal(
      <div className="del-overlay" onClick={() => !loading && onCancel()}>
         <div className="del-dialog" onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true">
            ...
            <div className="del-head">
               ...
               <h2 className="del-title">Retire <em>{serviceType.name}</em> from the register?</h2>
               <p className="del-lede">
                  The kind shall be struck from the register. Services bearing this
                  designation may require re&#8209;classification before use.
               </p>
            </div>
            ...
            <div className="del-foot">
               ...
               <button type="button" onClick={handleConfirm} disabled={loading} className="del-btn del-btn--danger">
                  {loading ? (<><span className="del-spinner" /><span>Striking from record…</span></>) : (<><span>Retire Kind</span><span className="del-btn-arrow">&rarr;</span></>)}
               </button>
            </div>
         </div>
         <DeleteDialogStyles />
...
function RegisterStyles() {
   return (<style jsx global>{`
      :root {
         ...
         --reg-rose: #b85a3a; --reg-green: #6b7d3a;
      ...
      .reg-table { width: 100%; border-collapse: separate; border-spacing: 0; font-family: var(--reg-font); min-width: 860px; }
      ...
      .reg-td--value { width: 200px; }
      .reg-code {
         font-family: var(--reg-font);
         font-size: 0.78rem;
         letter-spacing: 0.08em;
         background: linear-gradient(180deg, rgba(168,127,43,0.05), rgba(168,127,43,0.02));
      ...
      .reg-td--desc { max-width: 420px; min-width: 240px; }
      .reg-td-desc { color: var(--reg-ink-2); font-weight: 300; font-style: italic; font-size: 0.9rem; line-height: 1.5; overflow: hidden; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; }
