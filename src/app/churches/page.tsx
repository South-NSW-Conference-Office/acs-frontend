...
import { Church, ChurchPagination } from '@/types/hierarchy';
...
function getConferenceName(church: ChurchWithExtras): string {
   ...
   if (typeof conf === 'object' && conf !== null && conf.name) return conf.name;
   if (typeof church.conferenceId === 'object' && church.conferenceId !== null) {
      const c = church.conferenceId as { name?: string };
...
export default function Churches() {
   const [churches, setChurches] = useState<ChurchWithExtras[]>([]);
   ...
   const [churchToDelete, setChurchToDelete] = useState<ChurchWithExtras | undefined>(undefined);
   ...
   const fetchChurches = useCallback(async () => {
      try {
         ...
         if (response?.success) {
            setChurches((response.data ?? []) as ChurchWithExtras[]);
   ...
   const handleDeleteChurch = async (church: ChurchWithExtras) => {
      try {
         ...
         if (response.success) {
            toast.success('Church retired', `${church.name} has been removed from the register.`);
            fetchChurches();
         } else {
   ...
   const toggleSelectAll = () => {
      if (selectedIds.size === churches.length) {
         setSelectedIds(new Set());
      } else {
         setSelectedIds(new Set(churches.map((c) => c._id)));
      }
   };
   ...
   const handleBulkDelete = async () => {
      ...
      for (const id of ids) {
         try {
            const response = await ChurchService.deleteChurch(id);
            if (response.success) {
               results.success++;
            } else {
               results.failed++;
               results.errors.push(response.message || `Failed to delete church ${id}`);
            }
         } catch (error) {
            results.failed++;
            results.errors.push(error instanceof Error ? error.message : `Failed to delete church ${id}`);
         }
      }

      if (results.success > 0) {
         toast.success('Bulk retirement complete', `${results.success} church(es) removed from the register.`);
      }
      if (results.failed > 0) {
         toast.error('Some removals failed', `${results.failed} church(es) could not be removed. ${results.errors[0]}`);
      }
   ...
   // Distinct conferences on this page — used as the counter sub-line.
   const conferencesInView = new Set(
      churches.map((c) => getConferenceName(c)).filter((n) => n && n !== 'Unattached')
   ).size;
         ...
         <div className="reg-root">
            ...
            <header className="reg-masthead">
               <div className="reg-mast-left">
                  ...
                  <h1 className="reg-title">
                     ...
                     <span className="reg-word reg-word--italic" style={{ animationDelay: '0.42s' }}>Churches</span>
                  </h1>
                  <p className="reg-subtitle">
                     A gathering of local parishes, each a house of worship and
                     a hearth from which teams and community services are sent.
                  </p>
               </div>

               <aside className="reg-mast-right">
                  <div className="reg-counter">
                     <div className="reg-counter-num">{String(pagination.total).padStart(2, '0')}</div>
                     <div className="reg-counter-label">Parishes on the Roll</div>
                     <div className="reg-counter-rule" />
                     <div className="reg-counter-sub">
                        <span>{conferencesInView}</span> conferences represented
                     </div>
                  </div>
               </aside>
            </header>
            ...
            <div className="reg-console">
               <div className="reg-console-left">
                  <div className="reg-search">
                     ...
                     <input
                        type="text"
                        placeholder="Search by parish, code, city, or correspondent…"
                        value={search}
            ...
            {loading ? (
               ...
               <div className="reg-empty">
                  ...
                  <h3 className="reg-empty-title">
                     {search ? 'No parish meets your search.' : 'The roll awaits its first inscription.'}
                  </h3>
                  <p className="reg-empty-body">
                     {search
                        ? 'Amend your query, or broaden it to include dormant parishes.'
                        : 'Begin by inscribing the first church to the register.'}
                  </p>
               </div>
            ) : (
                        ...
                        <tr>
                           <th className="reg-th reg-th--check" onClick={(e) => e.stopPropagation()}>
                              <label className="reg-check" onClick={(e) => { e.preventDefault(); toggleSelectAll(); }}>
                                 <input
                                    type="checkbox"
                                    checked={churches.length > 0 && selectedIds.size === churches.length}
                                    readOnly
                                 />
                           ...
                           <th className="reg-th">Parish</th>
                           <th className="reg-th">Conference</th>
                           <th className="reg-th">Seat</th>
                           <th className="reg-th">Pastor &amp; Contact</th>
                     ...
                     <tbody>
                        {displayedChurches.map((church, i) => {
                           ...
                           const goto = () => {
                              ...
                              if (selectedIds.size > 0) { toggleSelect(church._id); return; }
                           ...
                           return (
                              ...
                              >
                                 <td className="reg-td reg-td--check" onClick={(e) => e.stopPropagation()}>
                                    <label className="reg-check" onClick={(e) => { e.preventDefault(); toggleSelect(church._id); }}>
                                       <input
                                          type="checkbox"
                                          checked={selectedIds.has(church._id)}
                                          readOnly
                                       />
                                       <span className="reg-check-box">
                                          <svg viewBox="0 0 12 12" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
                                 ...
                                 <td className="reg-td reg-td--no">
                                    <span className="reg-no">{String(globalIndex).padStart(3, '0')}</span>
                                 </td>
                                          ...
                                          <div className="reg-avatar-inner">
                                             {church.primaryImage?.url ? (
                                                ...
                                                <div className="reg-avatar-fallback">
                                                   <span>{(church.name || 'P').charAt(0)}</span>
                                                </div>
                                             )}
                                          </div>
                                       </div>
                                       <div className="reg-td-union-text">
                                          <div className="reg-td-name">{church.name}</div>
                                          <div className="reg-td-path">
                                             {church.code ? <span className="reg-td-code">№ {church.code}</span> : church.hierarchyPath}
                                          </div>
                                       </div>
                                    </div>
                                 </td>
                                 ...
                                 <td className="reg-td reg-td--parent">
                                    <div className="reg-td-parent">{getConferenceName(church)}</div>
                                 </td>
                                 ...
                                 <td className="reg-td reg-td--state">
                                    <span className={`reg-state ${church.isActive ? 'on' : 'off'}`}>
            ...
            {!loading && pagination.total > 0 && (
               <div className="reg-pager">
                  <span className="reg-pager-range">
                     <em>{rangeStart}</em>&ndash;<em>{rangeEnd}</em> of {pagination.total}
                  </span>
                  <div className="reg-pager-nav">
                     <button
                        className="reg-pager-btn"
                     ...
                     {pageNumbers.map((p, idx) => p === '...' ? (
                        <span key={`ellipsis-${idx}`} className="reg-pager-ellipsis">&middot;&middot;&middot;</span>
                     ) : (
                        <button
                           key={p}
                           onClick={() => setPage(p as number)}
                           className={`reg-pager-num ${p === page ? 'active' : ''}`}
                        >
         ...
         <div className="del-dialog" onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true">
            ...
            <div className="del-head">
               ...
               <h2 className="del-title">
                  Retire <em>{count}</em> {count === 1 ? 'parish' : 'parishes'}?
               </h2>
               <p className="del-lede">
                  The selected entries shall be removed from the active register.
                  They may be reinstated by an administrator should circumstances change.
               </p>
            </div>
            ...
            <div className="del-foot">
               ...
               >
                  {loading ? (
                     ...
                     <>
                        <span>Retire {count} {count === 1 ? 'Parish' : 'Parishes'}</span>
...
function DeleteChurchDialog({
   ...
   isOpen: boolean;
   church: ChurchWithExtras | undefined;
   onCancel: () => void;
   onConfirm: () => void | Promise<void>;
}) {
   ...
   if (!isOpen || !church || !mounted) return null;
   ...
   const members = church.demographics?.membership?.total;
   const hasMembers = typeof members === 'number' && members > 0;

   const notes = [
      { label: 'Teams gathered here',      hint: 'to be reassigned' },
      { label: 'Services rendered here',   hint: 'to be reassigned' },
      { label: 'Members on the roll',      hint: hasMembers ? `${members} of record` : 'none noted' },
   ];
      ...
      <div className="del-overlay" onClick={() => !loading && onCancel()}>
         <div className="del-dialog" onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true">
            ...
            <div className="del-head">
               ...
               <h2 className="del-title">
                  Retire <em>{church.name}</em> from the register?
               </h2>
               <p className="del-lede">
                  The parish shall be removed from the active roll. Its record is
                  retained in the archive, and may be reinstated should the
                  congregation be gathered anew.
               </p>
            </div>
...
function RegisterStyles() {
   return (
      <style jsx global>{`
         ...
         .reg-th--no      { width: 68px; padding-left: 0.6rem; }
         ...
         .reg-td--no { padding-left: 0.6rem; width: 68px; }
         .reg-no {
            font-size: 0.72rem;
            font-weight: 400;
            letter-spacing: 0.1em;
         ...
         /* CORRESPONDENCE */
         .reg-td--corr { max-width: 260px; min-width: 200px; }
         .reg-td-pastor {
            color: var(--reg-ink);
            font-weight: 500;
            font-size: 0.9rem;
         ...
         .reg-pager-btn { min-width: 2.6rem; font-size: 1rem; }
         .reg-pager-btn:hover:not(:disabled),
         .reg-pager-num:hover:not(.active) {
            color: var(--reg-gold);
            border-color: rgba(168, 127, 43, 0.3);
         }
         .reg-pager-btn:disabled { opacity: 0.3; cursor: not-allowed; }
         .reg-pager-num.active {
            color: #fff;
            background: var(--reg-gold);
            border-color: var(--reg-gold);
         }
