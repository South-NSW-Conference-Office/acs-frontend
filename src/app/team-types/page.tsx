...
export default function TeamTypesPage() {
   ...
   const handleCreateTeamType = async (teamTypeData: CreateTeamTypeData) => {
      try {
         ...
         toast({ title: 'Team type inscribed', description: 'The kind has been added to the register.' });
         loadTeamTypes();
      } catch (error: unknown) {
   ...
   const confirmDeleteTeamType = async () => {
      ...
      try {
         ...
         toast({ title: 'Team type retired', description: `${teamTypeToDelete.name} removed from the register.` });
         loadTeamTypes();
   ...
   const toggleSelectAll = () => {
      if (selectedIds.size === filteredTeamTypes.length) {
         setSelectedIds(new Set());
      } else {
         setSelectedIds(new Set(filteredTeamTypes.map((t) => t._id)));
      }
   };
   ...
   const handleBulkDelete = async () => {
      ...
      if (results.success > 0) {
         toast({ title: 'Bulk retirement complete', description: `${results.success} kind(s) retired from the register.` });
      }
      if (results.failed > 0) {
         toast({ title: 'Some removals failed', description: `${results.failed} could not be retired. ${results.errors[0] || ''}`, variant: 'destructive' });
      }
   ...
   const totalTeams = teamTypes.reduce((acc, t) => acc + (t.teamCount ?? 0), 0);

   return (
      <AdminLayout
         title="Team Types"
         description="Manage team types for your organization"
         hideTitle={true}
         hideHeader={true}
      >
         <RegisterStyles />
            ...
            <header className="reg-masthead">
               <div className="reg-mast-left">
                  ...
                  <h1 className="reg-title">
                     ...
                     <span className="reg-word reg-word--italic" style={{ animationDelay: '0.42s' }}>Kinds</span>
                  </h1>
                  <p className="reg-subtitle">
                     An ordering of the team types by which teams are classified &mdash;
                     a taxonomy of ministry, service, and craft.
                  </p>
               </div>

               <aside className="reg-mast-right">
                  <div className="reg-counter">
                     <div className="reg-counter-num">{String(filteredTeamTypes.length).padStart(2, '0')}</div>
                     <div className="reg-counter-label">Kinds on Record</div>
                     <div className="reg-counter-rule" />
                     <div className="reg-counter-sub">
                        <span>{totalTeams}</span> teams classed
                     </div>
                  </div>
               </aside>
            </header>
            ...
            <div className="reg-console">
               <div className="reg-console-left">
                  <div className="reg-search">
                     ...
                     <input
                        type="text"
                        placeholder="Search kinds by name or designation…"
                        value={searchQuery}
            ...
            {loading ? (
               <div className="reg-skel-table">
                  <div className="reg-skel-header">
                     ...
                     <div className="reg-skel-line reg-skel-line--head" style={{ width: '30%' }} />
                  ...
                  {[0, 1, 2, 3, 4].map((i) => (
                     <div key={i} className="reg-skel-row" style={{ animationDelay: `${i * 0.08}s` }}>
                        ...
                        <div className="reg-skel-line" style={{ flex: 0.5 }} />
                        <div className="reg-skel-line" style={{ flex: 0.7 }} />
               ...
               <div className="reg-empty">
                  ...
                  <h3 className="reg-empty-title">
                     {searchQuery ? 'No kind meets your search.' : 'The register awaits its first inscription.'}
                  </h3>
                  <p className="reg-empty-body">
                     {searchQuery
                        ? 'Amend your query, or broaden it to include dormant kinds.'
                        : 'Begin by inscribing the first team type to the register.'}
                  </p>
               </div>
            ) : (
                        ...
                        <tr>
                           <th className="reg-th reg-th--check" onClick={(e) => e.stopPropagation()}>
                              <label className="reg-check" onClick={(e) => { e.preventDefault(); toggleSelectAll(); }}>
                                 <input
                                    type="checkbox"
                                    checked={filteredTeamTypes.length > 0 && selectedIds.size === filteredTeamTypes.length}
                                    readOnly
                                 />
                           ...
                           <th className="reg-th reg-th--num">Teams</th>
                           <th className="reg-th">Class</th>
                           <th className="reg-th">Standing</th>
                           <th className="reg-th reg-th--actions" aria-label="Actions" />
                        </tr>
                     </thead>
                     <tbody>
                        {filteredTeamTypes.map((teamType, i) => {
                           const count = teamType.teamCount ?? 0;
                           const goto = () => {
                              if (deleteModalOpen || editModalOpen || createModalOpen || showBulkDeleteConfirm) return;
                              if (selectedIds.size > 0) { toggleSelect(teamType._id); return; }
                              window.location.href = `/team-types/${teamType._id}`;
                           };
                           return (
                              <tr
                                 key={teamType._id}
                              ...
                              >
                                 <td className="reg-td reg-td--check" onClick={(e) => e.stopPropagation()}>
                                    <label className="reg-check" onClick={(e) => { e.preventDefault(); toggleSelect(teamType._id); }}>
                                       <input
                                          type="checkbox"
                                          checked={selectedIds.has(teamType._id)}
                                          readOnly
                                       />
                                       <span className="reg-check-box">
                                          <svg viewBox="0 0 12 12" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
                                 ...
                                 <td className="reg-td reg-td--union">
                                    <div className="reg-td-union">
                                       ...
                                       <div className="reg-td-union-text">
                                          <div className="reg-td-name">{teamType.name}</div>
                                       </div>
                                    </div>
                                 </td>
                                 ...
                                 <td className="reg-td reg-td--desc">
                                    {teamType.description ? (
                                       <div className="reg-td-desc">{teamType.description}</div>
                                    ) : (
                                 ...
                                 <td className="reg-td reg-td--class">
                                    <span className={`reg-class ${teamType.isDefault ? 'reg-class--default' : ''}`}>
                                       {teamType.isDefault ? 'Default' : 'Custom'}
                                    </span>
                                 </td>

                                 <td className="reg-td reg-td--state">
                                    <span className={`reg-state ${teamType.isActive ? 'on' : 'off'}`}>
                                 ...
                                 >
                                    <RowActionsMenu actions={[
                                       { label: 'Edit', onClick: () => { setEditingTeamType(teamType); setEditModalOpen(true); } },
                                       { label: 'Retire', onClick: () => { setTeamTypeToDelete(teamType); setDeleteModalOpen(true); }, variant: 'danger' },
                                    ]} />
                                 </td>
                              </tr>
                           );
         ...
         <CreateTeamTypeModal
            ...
            onOpenChange={(open) => {
               ...
               if (!open) setEditingTeamType(null);
            }}
         ...
         <DeleteTeamTypeDialog
            isOpen={deleteModalOpen}
            teamType={teamTypeToDelete}
            onCancel={() => { setDeleteModalOpen(false); setTeamTypeToDelete(null); }}
         ...
         <div className="del-dialog" onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true">
            ...
            <div className="del-head">
               ...
               <h2 className="del-title">
                  Retire <em>{count}</em> {count === 1 ? 'kind' : 'kinds'}?
               </h2>
               <p className="del-lede">
                  The selected entries shall be removed from the register. Teams
                  bearing these kinds may require reassignment.
               </p>
            </div>
            <div className="del-foot">
               ...
               <button type="button" onClick={onConfirm} disabled={loading} className="del-btn del-btn--danger">
                  {loading ? (
                     ...
                     <><span>Retire {count} {count === 1 ? 'Kind' : 'Kinds'}</span><span className="del-btn-arrow">&rarr;</span></>
                  )}
               </button>
            </div>
         </div>
...
function DeleteTeamTypeDialog({
   ...
   if (!isOpen || !teamType || !mounted) return null;
   ...
   const notes = [
      { label: 'Teams classed by this kind', hint: teamCount > 0 ? `${teamCount} of record` : 'none noted' },
      { label: 'Default designation',         hint: teamType.isDefault ? 'system-defined' : 'custom' },
   ];
      ...
      <div className="del-overlay" onClick={() => !loading && onCancel()}>
         <div className="del-dialog" onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true">
            ...
            <div className="del-head">
               ...
               <h2 className="del-title">
                  Retire <em>{teamType.name}</em> from the register?
               </h2>
               <p className="del-lede">
                  The kind shall be removed from the active register. Teams bearing
                  this designation may require reassignment to another kind.
               </p>
            </div>
            ...
            <div className="del-reqs">
               ...
               {teamCount > 0 && (
                  <p className="del-reqs-warn">
                     ...
                     <strong>{teamCount}</strong> {teamCount === 1 ? 'team bears' : 'teams bear'} this
                     designation and may require reassignment before the kind is retired.
                  </p>
               )}
            </div>
            ...
            <div className="del-foot">
               ...
               <button type="button" onClick={handleConfirm} disabled={loading} className="del-btn del-btn--danger">
                  {loading ? (
                     ...
                     <><span>Retire Kind</span><span className="del-btn-arrow">&rarr;</span></>
                  )}
               </button>
            </div>
         </div>
   ...
   return (
      <style jsx global>{`
         ...
         .del-reqs {
            ...
            padding-top: 1.4rem; margin-bottom: 1.8rem;
         }
         ...
         .del-reqs-warn {
            ...
            font-size: 0.82rem; color: var(--del-rose-d); line-height: 1.5;
         }
         .del-reqs-warn strong { font-weight: 600; color: var(--del-rose); }
         .del-reqs-warn-dot {
            width: 6px; height: 6px; border-radius: 50%;
            background: var(--del-rose); flex-shrink: 0;
...
function RegisterStyles() {
   return (
      <style jsx global>{`
         ...
         .reg-counter-sub {
            font-size: 0.8rem; color: var(--reg-ink-2); font-style: italic; font-weight: 300;
         }
         ...
         .reg-table {
            ...
            font-family: var(--reg-font); min-width: 820px;
         }
         .reg-th {
         ...
         .reg-tr:hover .reg-avatar--sm .reg-avatar-ring {
            opacity: 1; transform: rotate(22deg);
         }
         ...
         .reg-td-desc {
            color: var(--reg-ink-2); font-weight: 300;
         ...
         .reg-td--class { width: 120px; }
         .reg-class {
            display: inline-block;
            font-size: 0.6rem; letter-spacing: 0.22em; text-transform: uppercase;
            font-weight: 500; color: var(--reg-ink-3);
